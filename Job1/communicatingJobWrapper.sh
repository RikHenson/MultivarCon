...
# SLURM_JOBID             - number of nodes allocated to Slurm job
# SLURM_NNODES            - number of hosts allocated to Slurm job
# SLURM_NODELIST          - list of hostnames allocated to Slurm job
...
chooseSmpdHosts() {
    ...
    # SLURM_NODELIST is required: the following line either echoes the value, or aborts.
    ...
    #
    # Our source of information is SLURM_NODELIST in the form:
    #   cnode[136,138],cnode[140-43],cnode157
    #
    # 'scontrol show hostname ${SLURM_NODELIST}' produces multi-line list of hostnames:
    #   node136
    #   node138
    #   node140
    #   ...
    ...
    SMPD_HOSTS=`scontrol show hostname ${SLURM_NODELIST} | tr '\n', ' '`
}
...
chooseSmpdPort() {
    ...
    # Extract the numeric part of SLURM_JOBID using sed to choose unique port for SMPD to run on.
    # Assumes SLURM_JOBID starts with a number, such as: 15.slurm-server-host.domain.com
...
#
# Inputs:
#   SLURM_NNODES              Slurm environment variable: Number of nodes allocated to Slurm job
#
...
cleanupAndExit() {
    ...
    STOP_SMPD_CMD="srun --ntasks-per-node=1 --ntasks=${SLURM_NNODES} ${FULL_SMPD} -shutdown -phrase MATLAB -port ${SMPD_PORT}"
    echo $STOP_SMPD_CMD
    eval $STOP_SMPD_CMD
...
launchSmpds() {
    ...
    START_SMPD_CMD="srun --ntasks-per-node=1 --ntasks=${SLURM_NNODES} ${FULL_SMPD} -phrase MATLAB -port ${SMPD_PORT} -debug 0 &"
    echo $START_SMPD_CMD
    eval $START_SMPD_CMD
    ...
    do
        ...
        if [ ${SLURM_NNODES} -eq ${NUM_HOSTS_FOUND} ] ; then
            SUCCESS=1
            break
    ...
    if [ $SUCCESS -ne 1 ] ; then
        if [ $NUM_HOSTS_FOUND -eq 0 ] ; then
            ...
            echo "Found SMPD processes running on only ${NUMHOSTS} of ${SLURM_NNODES} nodes.  Aborting."
